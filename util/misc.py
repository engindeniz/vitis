...
def get_video_mask(lengths, max_length):
    """Computes a batch of padding masks given batched lengths"""
    mask = 1 * (
...
def adjust_learning_rate(
    ...
    if args.different_lr_embedding_layers:
        optimizer.param_groups[1]["lr"] = args.embedding_layer_lr * gamma


def mask_tokens_text_dataloader(inputs, tokenizer, mlm_probability):
    """
    Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
    """
    ...
    if not torch.any(labels != -100):
        prob_matrix = probability_matrix[0]
        tmp_index = random.choice(torch.where(prob_matrix != 0)[0]).item()
        labels[0][tmp_index] = original_input[0][tmp_index]
        inputs[0][tmp_index] = tokenizer.convert_tokens_to_ids(tokenizer.mask_token)
    return inputs, labels
