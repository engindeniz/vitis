...
class VideoQA_Dataset(Dataset):
    def __init__(
        ...
        if train:  # for training remove answers that are not in vocab
            # print("Len of the dataset: ", len(self.data))
            ok = []
            ...
            # If answer is not in vocabulary, remove from training set
            # print("Len of the dataset after removed training examples:", len(self.data))
    ...
    def __getitem__(self, idx):
        ...
        # get video
        video, video_len = self._get_video_features(video_id)
...
def videoqa_collate_fn(batch):
    ...
    question = (
        [batch[i]["question"] for i in range(bs)]
        if isinstance(batch[0]["question"], str)
        else [
            [batch[i]["question"][j] for i in range(bs)]
            for j in range(len(batch[0]["question"]))
        ]
    )
...
def build_videoqa_dataset(dataset_name, split, args, tokenizer):
    ...
    if dataset_name in ["msvd", "msrvtt", "activitynet", "tgif"]:
        return VideoQA_Dataset(
            args=args,
            csv_path=csv_path,
            features_path=features_path,
            video_main_path=args.video_main_path,
            input_res=args.input_res,
            dataset_name=args.combine_datasets,
