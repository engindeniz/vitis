...
from model.transformer_encoder import Perceiver
from utils import initialization

from constants import PERCEIVER, MLP, EMD_CONTEXT_LAYER_W_TEXT_PROMPTS, EMD_CONTEXT_LAYER_WO_TEXT_PROMPTS, \
    EMD_CONTEXT_LAYER_NOT_EXISTS

"""https://huggingface.co/docs/transformers/v4.24.0/en/model_doc/deberta-v2#overview"""
"""https://github.com/huggingface/transformers/blob/v4.17.0/src/transformers/models/deberta_v2/modeling_deberta_v2.py"""
...
from torch.nn import CrossEntropyLoss, LayerNorm, Dropout
...
class XSoftmax(torch.autograd.Function):
    ...
    def backward(self, grad_output):
        ...
        inputGrad = _softmax_backward_data(grad_output, output, self.dim,
                                           output.dtype)
...
class DebertaV2Layer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, return_att=False, query_states=None, relative_pos=None,
                rel_embeddings=None, past_key_value=None):
...
class DebertaV2Encoder(nn.Module):
    ...
    def get_rel_pos(self, hidden_states, query_states=None, relative_pos=None, prompt_added_att_mask=None):
        size = hidden_states.size(-2) if prompt_added_att_mask is None else prompt_added_att_mask.size(-2)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if i == 0 and self.conv is not None:
                ...
                output_states = self.conv(hidden_states_tmp, output_states, input_mask)
...
class DisentangledSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x, attention_heads,
                             past_key_value=None):  # , text_prompt_type=None,text_prompt_location=None):
    ...
    def forward(
        ...
        attention_probs = XSoftmax.apply(attention_scores, softmax_mask,
                                         -1)
...
class DebertaV2Embeddings(nn.Module):
    ...
    def __init__(
        ...
        self.mapping_network = args.mapping_network
        # self.mlm_video = args.mlm_video
        ...
        if self.features_dim:
            if self.mapping_network == MLP:
                self.linear_video = nn.Linear(features_dim, config.hidden_size)
                self.linear_video.apply(self.init_weights)
            elif self.mapping_network == PERCEIVER:
                self.transformer_video = PerceiverMappingNetwork(args, config.hidden_size)
            else:
    ...
    def get_video_embedding(self, video):
        ...
        if self.mapping_network == MLP:
            ...
            video = self.transformer_video(video)
        else:
            raise NotImplementedError
        return video
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = self.position_ids[:,
                           text_prompts_length: seq_length + text_prompts_length]
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        text_prompts_length = past_key_values[0][0].shape[2] if past_key_values is not None else 0

        if attention_mask is None:
            attention_mask = torch.ones(((batch_size, seq_length + text_prompts_length)),
                                        device=device)

        if self.features_dim and video is not None:
            if video_mask is None:
                if self.args.mapping_network == PERCEIVER:
                    video_shape = self.args.mapping_prompt_num_tokens
                else:
        ...
        embedding_mask = attention_mask[:, text_prompts_length:].contiguous()
        ...
        embedding_output = self.embeddings(
            ...
            video=video,
            text_prompts_length=text_prompts_length,
...
class PrefixEncoder(torch.nn.Module):
    r'''
    https://github.com/THUDM/P-tuning-v2/blob/bb07da776e8bbeb1a8c061553a87734e950966d9/model/prefix_encoder.py#L1
    The torch.nn model to encode the prefix
    Input shape: (batch-size, prefix-length)
    Output shape: (batch-size, prefix-length, 2*layers*hidden)
    '''

    def __init__(self, projection_layer, num_prompt_tokens, text_model_hidden_size, projection_factor,
                 num_hidden_layers, text_intermediate_proj_layer_dim):

        super().__init__()
        self.prefix_projection = projection_layer

        if self.prefix_projection:
            # Use a two-layer MLP to encode the prefix
            prompt_dim = text_model_hidden_size // projection_factor
            self.embedding = nn.Parameter(torch.zeros(num_prompt_tokens, prompt_dim))
            initialization(module=self.embedding.data)
            if text_intermediate_proj_layer_dim > 0:
                raise NotImplementedError
            else:
                self.text_prompt_proj = torch.nn.Linear(prompt_dim,
                                                        num_hidden_layers * 2 * text_model_hidden_size)
                initialization(module=self.text_prompt_proj.weight)

        else:
            self.embedding = nn.Parameter(
                torch.zeros(num_prompt_tokens, num_hidden_layers * 2 * text_model_hidden_size))
            initialization(module=self.embedding.data)

    def forward(self):
        if self.prefix_projection:
            past_key_values = self.text_prompt_proj(self.embedding)
        else:
            past_key_values = self.embedding
        return past_key_values


class PerceiverMappingNetwork(nn.Module):
    def __init__(self, args, text_model_hidden_size):
        super().__init__()
        self.num_frames = args.num_frames
        self.features_dim = args.features_dim
        self.text_model_hidden_size = text_model_hidden_size
        self.perceiver_self_attn_num_layers = args.perceiver_self_attn_num_layers

        self.num_layer = args.mapping_network_num_transformer_layers
        self.transformer_hidden_size = args.transformer_hidden_size
        self.n_head = args.mapping_network_num_head_transformer_layers
        self.mapping_network_linear_after_transformer = args.mapping_network_linear_after_transformer
        self.mapping_network_transformer_dim_feedforward = args.mapping_network_transformer_dim_feedforward
        self.mapping_network_feedforward = args.mapping_network_feedforward
        self.mapping_prompt_num_tokens = args.mapping_prompt_num_tokens
        self.mapping_prompt_dropout_rate = args.mapping_prompt_dropout_rate
        self.temporal_pos_embed = nn.Parameter(torch.zeros(self.num_frames, self.transformer_hidden_size))
        initialization(module=self.temporal_pos_embed.data)
        self.visual_features_layer_norm = LayerNorm(self.transformer_hidden_size)

        self.perceiver_layers = nn.ModuleList([Perceiver(self.transformer_hidden_size, self.n_head,
                                                         self.mapping_network_transformer_dim_feedforward,
                                                         self.mapping_prompt_dropout_rate,
                                                         self.mapping_network_feedforward,
                                                         self.perceiver_self_attn_num_layers)
                                               for _ in
                                               range(self.num_layer)])

        self.linear_video_before = nn.Linear(self.features_dim, self.transformer_hidden_size)
        initialization(module=self.linear_video_before.weight)

        if self.mapping_network_linear_after_transformer:
            self.linear_video_after = nn.Linear(self.transformer_hidden_size, text_model_hidden_size)
            initialization(module=self.linear_video_after.weight)
    ...
    def prompt_initialization(self):
        ...
        self.prompt_dropout = Dropout(self.mapping_prompt_dropout_rate)

        prompt_proj_dim = self.transformer_hidden_size
        self.prompt_proj = nn.Identity()

        self.mapping_prompt_embeddings = nn.Parameter(
            torch.zeros(self.mapping_prompt_num_tokens, prompt_proj_dim))
        initialization(module=self.mapping_prompt_embeddings.data)

    def forward(self, video_features):

        video_features = self.linear_video_before(video_features)
        bs = video_features.shape[0]

        position_embeddings = self.temporal_pos_embed.expand(bs, *self.temporal_pos_embed.shape)
        video_features = self.visual_features_layer_norm(video_features) + position_embeddings

        prompts = self.prompt_dropout(
            self.prompt_proj(self.mapping_prompt_embeddings.expand(bs, *self.mapping_prompt_embeddings.shape)))
        x = prompts
        for i, layer in enumerate(self.perceiver_layers):
            x = layer(x, video_features)

        if self.mapping_network_linear_after_transformer:
            x = self.linear_video_after(x)

        return x


class PromptTuningVideoQA(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.args = args
        self.emd_context_layer_type = args.emd_context_layer_type
        self.text_config = text_config
        self.prompt_type = prompt_type
        self.visual_encoder = visual_encoder
        self.text_prompt_num_tokens = args.text_prompt_num_tokens
        self.text_prompt_dropout_rate = args.text_prompt_dropout_rate
        self.dropout = dropout
        self.ft_layer_norm = args.ft_layer_norm
        ...
        self.lm_predictions = DebertaV2OnlyMLMHead(text_config)

        state_dict = load_state_dict(args.deberta_model_path)

        deberta_state_dict = {k.replace("deberta.", ""): v for k, v in state_dict.items()}
        self.deberta.load_state_dict(
            deberta_state_dict, strict=False)

        lm_predictions_state_dict = {k.replace("lm_predictions.", ""): v for k, v in state_dict.items()}
        self.lm_predictions.load_state_dict(lm_predictions_state_dict,
                                            strict=False)
        ...
        if self.prompt_type in ['text', 'visual_text']:
            self.dropout_text_prompt = torch.nn.Dropout(self.text_prompt_dropout_rate)
            self.n_deberta_layer = text_config.num_hidden_layers
            self.n_deberta_embedding_size = getattr(text_config, "embedding_size", text_config.hidden_size)
            self.n_deberta_attention_heads = text_config.num_attention_heads
            self.n_embd = self.n_deberta_embedding_size // self.n_deberta_attention_heads

            self.prefix_encoder = PrefixEncoder(projection_layer=args.text_prompt_projection_layer,
                                                num_prompt_tokens=self.text_prompt_num_tokens,
                                                text_model_hidden_size=self.n_deberta_embedding_size,
                                                projection_factor=args.text_prompt_projection_factor,
                                                num_hidden_layers=self.n_deberta_layer,
                                                text_intermediate_proj_layer_dim=args.text_intermediate_proj_layer_dim,
                                                )
        ...
        if freeze_lm:
            for n, p in self.deberta.named_parameters():
    ...
    def set_input_embeddings(self, new_embeddings):
        self.deberta.embeddings.word_embeddings = new_embeddings
    ...
    def _tie_or_clone_weights(self, output_embeddings, input_embeddings):
        ...
        if self.text_config.torchscript:
    ...
    def set_answer_embeddings_for_ttp(self, a2tok, n_ans, freeze_last=True):
        a2v = self.deberta.embeddings.word_embeddings(a2tok)
        # pad_token_id = getattr(self.config, "pad_token_id", 0)
        pad_token_id = getattr(self.text_config, "pad_token_id", 0)
        sum_tokens = (a2tok != pad_token_id).sum(1, keepdims=True)  # n_ans
        if len(a2v) != n_ans:  # reinitialize the answer embeddings
            assert not self.training
            n_ans = len(a2v)
            self.answer_embeddings = nn.Embedding(
                n_ans, self.deberta.embeddings.embedding_size
            ).to(self.deberta.device)  # self.device
            self.answer_bias.requires_grad = False
            self.answer_bias.resize_(n_ans)
        self.answer_embeddings.weight.data = torch.div(
            (a2v * (a2tok != pad_token_id).float()[:, :, None]).sum(1),
            sum_tokens.clamp(min=1),
        )  # n_ans
    ...
    def emd_context_layer(self, encoder_layers, z_states, attention_mask, encoder, past_key_values=None):
        if attention_mask.dim() <= 2:
            extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        ...
        if not self.text_config.position_biased_input:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.args.prompt_save:
            torch.save(past_key_values,
                       os.path.join(self.args.prompt_save_path, "saved_text_prompt_embeddings.pth"))
        past_key_values = past_key_values.expand(batch_size, -1,
                                                 -1)
        past_key_values = past_key_values.view(
            batch_size,
            self.text_prompt_num_tokens,
            self.n_deberta_layer * 2,
            self.n_deberta_attention_heads,
            self.n_embd
        )
        past_key_values = self.dropout_text_prompt(past_key_values)
    ...
    def forward(
        ...
        if self.prompt_type in ["text", "visual_text"]:
            ...
            text_prompts = None
            prefix_attention_mask = torch.ones(batch_size, self.text_prompt_num_tokens).to(self.deberta.device)
        ...
        if self.args.prompt_save:
            if os.path.isfile(os.path.join(self.args.prompt_save_path,
                                           "saved_mapping_network_prompt_embeddings.pth")) and os.path.join(
                self.args.prompt_save_path, "saved_mapping_network_deep_prompt_embeddings.pth") and os.path.join(
                self.args.prompt_save_path, "saved_text_prompt_embeddings.pth"):
                sys.exit(1)
        ...
        if labels is not None:
            if (
                ...
                video_shape = video_mask.size()
                video_labels_tmp = torch.tensor(
                    [[-100] * video_shape[1]] * video_shape[0],
                    dtype=torch.long,
                    device=labels.device,
                )

                labels = torch.cat([video_labels_tmp, labels], 1)

        if self.emd_context_layer_type == EMD_CONTEXT_LAYER_NOT_EXISTS:
            sequence_output = outputs[0]

        elif self.emd_context_layer_type == EMD_CONTEXT_LAYER_W_TEXT_PROMPTS:

            past_key_values_tmp = past_key_values[-1]
            ...
            sequence_output = modified[-1]
        elif self.emd_context_layer_type == EMD_CONTEXT_LAYER_WO_TEXT_PROMPTS:
        ...
        prediction_scores = self.lm_predictions(sequence_output, embeddings, bias)
        ...
        if labels is not None:
            ...
            masked_lm_loss = loss_fct(
                prediction_scores.view(-1, self.text_config.vocab_size),
                labels.view(-1),
            )
